...
export const CustomConversationMessageSwipeable = memo(function CustomConversationMessageSwipeable({
  ...
  const panGesture = useMemo(
    () =>
      Gesture.Pan()
        .activeOffsetX([10, 1000]) // Small offset to start gesture
        .onBegin((event) => {
          // Check if the gesture started too close to the left edge
          isGestureAllowed.value = event.x > LEFT_EDGE_EXCLUSION
        })
        .onUpdate((event) => {
          // Only process if gesture is allowed
          if (!isGestureAllowed.value) return

          // Apply friction and clamp to max distance
          const rawTranslation = event.translationX / SWIPE_FRICTION
        ...
        .onEnd(() => {
          // Only trigger if gesture was allowed
