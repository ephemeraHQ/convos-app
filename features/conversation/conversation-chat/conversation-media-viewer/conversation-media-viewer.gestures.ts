...
/**
 * Custom hook for creating gesture handlers for the media viewer
 */
export function useGestureHandlers(props: IGestureHandlerProps) {
  const { 
    animState, 
    toggleControls, 
    onDismiss 
  ...
  // Extract animation values for cleaner code
  const { 
    scale, savedScale, 
    translateX, translateY, 
    savedTranslateX, savedTranslateY,
    backgroundOpacity,
    isDismissing,
    dismissAnimation
  } = animState
