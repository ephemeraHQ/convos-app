...
func _getKeychainValue(forKey: String, requireAuthentication: Bool? = nil) -> String? {
  ...
  log(
    ...
    type: .debug, category: "keychainRead")
  var item: CFTypeRef?
  let status = SecItemCopyMatching(query as CFDictionary, &item)

  log(
    "SecItemCopyMatching status: \(status) for key: \(forKey)", type: .debug,
    category: "keychainRead")

  switch status {
  case errSecSuccess:
    guard let itemData = item as? Data else {
      log(
        "Keychain item found for key: \(forKey), but failed to cast to Data.", type: .error,
        category: "keychainRead")
      return nil
    }
    log(
      "Successfully retrieved and cast keychain item for key: \(forKey).", type: .debug,
      category: "keychainRead")
  ...
  default:
    log(
      ...
      type: .error, category: "keychainRead")
    return nil
  }
}
...
func getKeychainValue(forKey: String) -> String? {
  ...
  if let unauthenticatedItem = _getKeychainValue(forKey: forKey, requireAuthentication: false) {
    log(
      "Found unauthenticated keychain item for key: \(forKey)", type: .debug,
      category: "keychainRead")
    return unauthenticatedItem
  }
  if let authenticatedItem = _getKeychainValue(forKey: forKey, requireAuthentication: true) {
    log(
      "Found authenticated keychain item for key: \(forKey)", type: .debug, category: "keychainRead"
    )
  ...
  if let legacyItem = _getKeychainValue(forKey: forKey) {  // This calls _getKeychainValue with requireAuthentication = nil
    log(
      "Found legacy (auth unspecified) keychain item for key: \(forKey)", type: .debug,
      category: "keychainRead")
    return legacyItem
  }
  log(
    "No keychain item found for key: \(forKey) after trying all methods.", type: .debug,
    category: "keychainRead")
  return nil
}
...
func setKeychainValue(value: String, forKey: String) throws -> Bool {
  ...
  log(
    "Attempting to set keychain value for key: \(forKey). Query: \(query as NSDictionary)",
    type: .debug, category: "keychainWrite")
  ...
  case errSecSuccess:
    log(
      "Successfully added keychain item for key: \(forKey)", type: .debug, category: "keychainWrite"
    )
    return true
  case errSecDuplicateItem:
    log(
      "Keychain item for key: \(forKey) already exists, attempting to update. Status: errSecDuplicateItem (\(status))",
      type: .debug, category: "keychainWrite")
    return try updateKeychainValue(value: value, forKey: forKey)
  default:
    log(
      "Failed to add keychain item for key: \(forKey). Status: \(status), OSStatus: \(status.description)",
      type: .error, category: "keychainWrite")
    return false
  }
}
...
func updateKeychainValue(value: String, forKey: String) throws -> Bool {
  ...
  log(
    "Attempting to update keychain value for key: \(forKey). Query: \(query as NSDictionary), Updates: \(updateDictionary as NSDictionary)",
    type: .debug, category: "keychainWrite")
  ...
  if status == errSecSuccess {
    log(
      "Successfully updated keychain item for key: \(forKey)", type: .debug,
      category: "keychainWrite")
    return true
  } else {
    log(
      "Failed to update keychain item for key: \(forKey). Status: \(status), OSStatus: \(status.description)",
      type: .error, category: "keychainWrite")
    return false
  }
}
