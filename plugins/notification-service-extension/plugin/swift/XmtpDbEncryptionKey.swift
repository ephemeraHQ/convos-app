...
func getDbEncryptionKey(ethAddress: String) -> Data? {
    ...
    if let keyFromKeychain = getKeychainValue(forKey: keychainKeyPrefix + ethAddress) {
        log(
            "Successfully retrieved DB encryption key from keychain", type: .debug,
            category: "keychain")
    ...
    if dbEncryptionKeyString == nil {
        ...
        log(
            "Fetching backup key for address \(ethAddress) from MMKV with key: \(key)",
            type: .debug, category: "mmkv")
    ...
    if dbEncryptionKeyString == nil {
        if let key = getSharedDefaultsValue(key: sharedDefaultsKey + ethAddress) {
            log(
                "Retrieved DB encryption key from SharedDefaults", type: .debug,
                category: "sharedDefaults")
    ...
    guard let finalDbEncryptionKeyString = dbEncryptionKeyString else {
        log(
            "Failed to get DB encryption key from keychain, MMKV backup, and SharedDefaults",
            type: .error)
        return nil
    }
